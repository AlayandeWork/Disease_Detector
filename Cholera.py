...
def symptom(nrows):
    ...
    Symptoms = ["Diarrhea", "Dehydration", "Abdominal Cramps", "Irritability", "Restlessness", "Lethargy", "Vomiting"]
    symptom_data = {}
...
Cholera = {'id': ids, 'Gender': genders}
Cholera.update(symptom_data)

Cholera_df = pd.DataFrame(Cholera)

# Calculate the Severity column based on symptom_data
Cholera_df['Severity'] = Cholera_df.iloc[:, 2:].apply(calculate_severity, axis=1)

Cholera_df.to_csv('Cholera_Data.csv', index=False)
